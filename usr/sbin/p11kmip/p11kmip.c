			     ...
			     bool *self_signed, bool *valid)
{
	...
	if (cert == NULL) {
		warnx("Failed to read certificate PEM file '%s'",
			   cert_file);
		return -EIO;
	}
...
static CK_RV init_kmip(void)
{
    ...
    char server_cert_pem_path[40], server_pubkey_pem_path[40];
    rc = CKR_OK;
    ...
    /*
        TODO:
          - probably just copy most of the code for the checking and
            whatnot from zKey, since that's probably all set up
            to work the way that Ingo wants it. No need to reinvent 
            the wheel
    */
    ...
    if (rc != CKR_OK) {
        warnx("Failed to check KMIP server certificate '%s'", P11KMIP_SERVER_CERT_PATH);
        goto done;
    }
    ...
    if (opt_tls_trust_server == false) {
        ...
        if (rc != CKR_OK) {
            warnx("Failed to print KMIP server certificate '%s'", P11KMIP_SERVER_CERT_PATH);
            goto done;
        }
                                             ...
                                             unwrapped_key_handle)
{
    ...
    unwrapped_default_templatecount = 9 + iscca;
